...
class SearchProcessor:
    ...
    def get_medicine_registrations(self, medicine: Medicine) -> List[Dict[str, Union[str, int]]]:
        description_temp = medicine.extracted_substances if medicine.extracted_substances is not None else medicine.description
        registrations = self.anvisa_search.get_register(description_temp, medicine.brand)
        if registrations:
            self.logger.info(f'Found {len(registrations)} candidates in ANVISA data\n')
        else:
            self.logger.info('No candidates found in ANVISA data, searching in SMERP')
            registrations = self.smerp_search.get_data_from_smerp(description_temp, medicine.brand)
            if registrations:
                self.logger.info(f'Found {len(registrations)} candidates in SMERP data\n')
            else:
